...
function getCase(start,len,tag){
	var act = {"act":"getPublish","start":start,"len":len,"order":order,"direct":direct};
	if(tag){
	...
	$.ajax({
		...
		success:function(r){
			...
			for (var i = 0; i < data.length; i++) {
				...
				if(data[i].like==-1)
					$("#casearea").append("<tr><td><a href='jobdetail.html?id="+data[i].id+"'>"+data[i].title+"</a></td><td>"+data[i].salary+"</td><td>"+updatetime+"</td><td><a href='resumelist.html?id="+data[i].id+"'>"+data[i].num+"</a></td><td>"+data[i].views+"</td>");
